...
import { simulateDispatch, rankHospitals } from "@/lib/nagraksha";

type BusEventMap = {
  IncidentCreated: (incidentId: string, payload: any) => void;
  DispatchAttempted: (incidentId: string, payload: any) => void;
  DispatchAccepted: (incidentId: string, payload: any) => void;
  IncidentStateChanged: (incidentId: string, payload: any) => void;
  IncidentClosed: (incidentId: string, payload: any) => void;
};
...
class NagRakshaBus extends EventEmitter {
  ...
  on<K extends keyof BusEventMap>(ev: K, fn: BusEventMap[K]) {
    return super.on(ev, fn as any);
  }
  emit<K extends keyof BusEventMap>(ev: K, ...args: Parameters<BusEventMap[K]>) {
    return super.emit(ev, ...(args as any[]));
  }
}
...
function registerHandlers(bus: NagRakshaBus) {
  ...
  bus.on(
    "IncidentCreated",
    safe(async (incidentId: string, payload: any) => {
    await audit({
      incidentId,
      actor: "system",
    ...
    const sim = simulateDispatch({ lat: payload.lat, lng: payload.lng });
    const lanes: { category: "TRAINED" | "RESCUE" | "AMBULANCE"; attempts: any[] }[] = [
    ...
    hospitals.map((h) => {
      ...
      return {
        ...
        stock: {
          ...
          status: (s?.status as any) ?? "UNKNOWN",
